...
namespace WebAPI.Controllers
{
	public class EmailNotificationsController : ApiController
	{
		...
		[Route("api/EmailNotifications/GetAll/{userID}")]
		[HttpGet]
		[AllowAnonymous]
		...
		public HttpResponseMessage GetAll(string userID)
		{
			...
			var result = _emailNotificationsRepo.Find(
				x => x.To == userID).OrderByDescending(x => x.Id).Take(10).ToList();
			//var result = _emailNotificationsRepo.GetAll().ToList();
		...
		[Route("api/EmailNotifications/DeleteEmail/{ids}")]
		[HttpPost]
		[AllowAnonymous]
		public HttpResponseMessage DeleteEmail(string ids)
		{
			...
			if (ids.Split(',').Length > 1)
			{
				...
				{
					var result = _emailNotificationsRepo.Get(Convert.ToInt32(ids.Split(',')[i]));
			...
			{
				var result = _emailNotificationsRepo.Get(Convert.ToInt32(ids));
				result.IsDeleted = true;
				var msgString = _emailNotificationsRepo.Update(result);
		...
		public HttpResponseMessage GetTestFile()
		{
			...
			var localFilePath = System.Web.HttpContext.Current.Server.MapPath("~/EmailNotifications/" + fileName); ;
